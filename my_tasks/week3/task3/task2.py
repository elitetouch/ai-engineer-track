...
while True:
    ...
    # Age input with try/except
    try:
        student_detail["age"] = int(input("Enter your age: "))
    except ValueError:
        print("\nInvalid input! Age must be a number. Please try again.\n")
        continue  # restart the loop for valid input

    student_detail["country"] = input("Enter your country: ").strip().lower()
    student_detail["university"] = input("Enter your university or 'non' if not enrolled: ").strip()
    student_detail["scholarship"] = input("Have you received scholarship from an oil & gas company? (yes/no): ").strip().lower()
    student_detail["subjects"] = ['Maths', 'English', 'Physics', 'Chemistry', 'Biology']
