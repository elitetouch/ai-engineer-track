# Admission Eligibility Checker updated with control flow, input validation and retry option
# created by Peter Okonmah

print("=== Admission Eligibility Checker (2025/2026) ===\n")

while True:
    # Age input (must be a number)
    age = input("Enter your age: ")
    while not age.isdigit():
        age = input("Invalid input! Enter a valid number for age: ")
    age = int(age)

    # UTME score input
    utme_score = input("Enter your UTME score: ")
    while not utme_score.isdigit():
        utme_score = input("Invalid input! Enter a valid number for UTME score: ")
    ...
    # O'Level passes
    o_level_passes = input("How many O'Level credits do you have at one sitting?: ")
    while not o_level_passes.isdigit():
        o_level_passes = input("Invalid input! Enter a valid number for O'Level credits: ")
    o_level_passes = int(o_level_passes)
    ...
    # Post-UTME score
    post_utme_score = input("Enter your Post-UTME score (out of 400): ")
    while not post_utme_score.isdigit():
        post_utme_score = input("Invalid input! Enter a valid number for Post-UTME score: ")
    ...
    while retry not in ["yes", "no"]:
        retry = input("Please answer 'yes' or 'no': ").lower()
    if retry != "yes":
